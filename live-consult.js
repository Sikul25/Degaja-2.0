(() => {
  ...
  function start(){
    ...
    const a=getSelectedAdvisor();
    if(a.available===false){host.innerHTML=`<div class="degaja-call-panel"><h4 style="color:#18334a">${t('live.notAvailableTitle')}</h4><p class="degaja-live-copy">${fmt(t('live.notAvailableDesc'),{name:esc(a.name)})}</p><a href="#oracle" class="dark" style="display:inline-block;text-decoration:none;border-radius:999px;padding:12px 18px;font-weight:800">${t('live.tryOracleBtn')}</a></div>`;return;}
    const c=getCredits();
