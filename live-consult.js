(() => {
  ...
  function card(){
    ...
    el.innerHTML=`<div class="degaja-live-head"><div><div class="eyebrow">${t('live.eyebrow')}</div><h3>${t('live.title')}</h3></div><span class="degaja-live-badge">${t('live.badge')}</span></div><p class="degaja-live-copy">${t('live.intro')}</p><div class="degaja-advisor-picker"><label for="degajaAdvisorSelect">${t('live.advisorLabel')}</label><select id="degajaAdvisorSelect">${advisorOptions()}</select></div><div class="degaja-live-actions" style="margin-top:12px"><button class="gold" id="degajaLiveStart">${t('live.startBtn')}</button><button class="soft" id="degajaAdvisorToggle">${t('live.advisorAccessBtn')}</button></div><div class="degaja-live-credit" id="degajaLiveCredit"></div><div class="degaja-call" id="degajaCall"></div><div class="degaja-advisor" id="degajaAdvisor"><h4 style="color:#18334a">${t('live.advisorAccessTitle')}</h4><p class="degaja-live-copy">${t('live.advisorAccessDesc')}</p><input id="degajaAdvisorCode" maxlength="6" inputmode="numeric" placeholder="${t('live.sessionCodePlaceholder')}"><button class="dark" id="degajaAdvisorJoin">${t('live.acceptCallBtn')}</button><div class="degaja-status" id="degajaAdvisorStatus"></div><audio id="degajaAdvisorAudio" autoplay controls></audio><div class="degaja-transcript" id="degajaAdvisorTranscript" style="display:none"></div></div><div class="degaja-privacy">${t('live.privacyFooter')}</div>`;
    wrap.appendChild(el);
